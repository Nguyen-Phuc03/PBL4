...
public class Model_File_Sender {
    ...
    public void initSend(String filename,String path) throws IOException {
        socket.emit("send_to_user", message.toJsonObject(), new Ack() {
            ...
            public void call(Object... os) {
                if (os.length > 0) {
                    ...
                    int fileID = (int) os[0];
                    try {
                        startSend(fileID,filename,path);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }else{
    ...
    public void startSend(int fileID,String filename,String path) throws IOException {
        this.fileID = fileID;
        if (event != null) {
            event.onStartSending();
        }
        sendingFile(filename,path);
    }

    private void sendingFile(String filename,String path) throws IOException {
